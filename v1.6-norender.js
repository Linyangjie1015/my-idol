    ...
    for (var ci = 0; ci < compKeys.length; ci++) {
        ...
        for (var gi = 0; gi < gKeys.length; gi++) {
            if (comp.groups[gKeys[gi]].name === gameState.player.group) {
                group = comp.groups[gKeys[gi]]; break;
            }
        }
        if (group) break;
    }
...
window.onerror = function(msg, url, line, col, err) {
    ...
    document.body.innerHTML = '<div style="padding:20px;font-family:monospace;white-space:pre-wrap;word-break:break-all;background:#fff;color:#000;">ERROR: ' + msg + '\nLINE: ' + line + '\nURL: ' + url + '\nSTACK:\n' + stack + '</div>';
    // ORIG = '<div style="padding:40px;text-align:center;font-family:sans-serif;">' +
...
document.title = 'V1.6 LOADED OK - NO RENDER';